...
export default function Playground() {
  ...
  const { user, credits, subscription, spendCredit } = useAuth();
  const examples = [
  ...
  const generateSound = async () => {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        ...
        toast.error(
          `Failed to generate sound: ${error.message || "Unknown error"}`
        );
      }
    } finally {
      setLoading(false);
      ...
      <main className="flex-grow container mx-auto px-4 py-12 md:py-24">
        <div className="max-w-5xl mx-auto">
          <section className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold mb-8 text-[#7e6bff]">
            ...
            {user ? (
              <div className="mt-4 flex justify-center items-center gap-3">
                <Badge
                  variant={
                    subscription === "premium" ? "destructive" : "default"
                  }
              ...
              <div className="mt-4 flex justify-center items-center gap-3">
                <Link href="/signup">
                  <Button
                    variant="primary"
                    className="bg-[#4c65ff] hover:bg-[#3b52e4]"
                  >
                ...
                <Link href="/login">
                  <Button
                    variant="outline"
                    className="border-[#2a3655] bg-[#1a2235] hover:bg-[#2a3655]"
                  >
                    Login
                  </Button>
                </Link>
            ...
            <CardFooter className="flex flex-wrap justify-center gap-2 pt-2 pb-6 border-t border-[#1e2a45]">
              <button
                ...
                className={`relative flex items-center justify-between cursor-pointer
                  w-full sm:w-auto rounded-lg font-medium
              ...
              >
                ...
                <Badge className="ml-4 bg-white/20 text-white border-none py-1 px-3 flex items-center gap-1 backdrop-blur-sm">
                  <Clock size={14} />
                  <span className="font-medium">{credits} credits</span>
                </Badge>

                {/* Animated background effect */}
                <div className="absolute inset-0 w-full h-full transition-all duration-300 scale-x-0 origin-left group-hover:scale-x-100 bg-white/10"></div>
              </button>
                    ...
                    >
                      <CardContent className="p-4 flex justify-between items-center">
                        <div className="flex-1">
                          <div className="mb-2 font-medium group-hover:text-[#7e6bff] transition-colors text-white">
                            {item.text}
                          </div>
